...
const originalData = JSON.parse(fs.readFileSync('extracted_books.json', 'utf8'));
...
if (match) {
    ...
    currentBooks = currentBooks.map(book => {
        ...
        // Need to check how titles match. Let's try exact first, or similar.
        const originalBook = originalData.find(b => b.title.includes(book.title) || book.title.includes(b.title));
        if (originalBook && originalBook.pdf && originalBook.pdf.includes('archive.org')) {
